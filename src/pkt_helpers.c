...
/**
 * @brief Locate IPv4 header in an mbuf and return L2 length.
 */
int
get_ipv4_hdr(struct rte_mbuf *m, struct rte_ipv4_hdr **ip_hdr, uint32_t *l2_len)
{
    ...
    /* Fast path: packet starts with IPv4 header (no Ethernet). */
    if ((data[0] >> 4) == 4) {
        *l2_len = 0;
    ...
    /* Standard Ethernet + IPv4.
     * TODO: handle VLAN tags (802.1Q/802.1AD) before checking EtherType.
     */
...
/**
 * @brief Locate IPv6 header in an mbuf and return L2 length.
 */
int
...
/**
 * @brief Print a slice of payload bytes (ASCII + hex for non-printables).
 */
void
...
/**
 * @brief Compute IPv4 TCP/UDP payload bounds.
 */
int
                      ...
                      uint32_t *payload_offset, uint32_t *payload_len, uint8_t *proto)
{
    ...
    *proto = ip_hdr->next_proto_id;
    ...
    /* TCP: compute payload after variable header size. */
    ...
    /* UDP: payload length is bounded by both IP and UDP lengths. */
...
/**
 * @brief Compute IPv6 TCP/UDP payload bounds (handles fragment header).
 */
int
                        ...
                        uint32_t *payload_offset, uint32_t *payload_len, uint8_t *proto)
{
    ...
    /* Skip IPv6 fragment extension header when present. */
...
/**
 * @brief Print L4 payload for IPv4 packets (debug).
 */
void
...
/**
 * @brief Dump first bytes of a packet for debugging.
 */
void
